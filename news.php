...
</style><style>.navbar,.nav-menu,.nav-menu a{transition:none!important.news-pagination,.news-pagination *{opacity:1!important;visibility:visible!important;display:flex!important}.news-pagination .pagination-btn{display:inline-flex!important}}</style>
...
<script src="/js/main.js?v=20260610"></script>
    ...
    <script>
        ...
        // 从服务器API加载分类和文章数据（与手机端一致，从数据库读取）
                ...
                async function loadNewsFromServer(page) {
            page = page || currentPage;
            var ts = Date.now();
            var apiUrl = currentCategoryId
                ? '/mobile/api/news.php?category_id=' + currentCategoryId + '&page=' + page + '&limit=10&t=' + ts
                : '/mobile/api/news.php?page=' + page + '&limit=10&t=' + ts;
            try {
                var r = await fetch(apiUrl, { cache: 'no-store' });
                if (!r.ok) return null;
                var d = await r.json();
                if (d.success && d.data) { if (d.data.total) totalArticles = d.data.total; return d.data.news || []; }
                return null;
            } catch(e) { return null; }
        }
        ...
        function loadCategories() {
            ...
            // 保存服务端渲染的分类作为兜底，避免使用硬编码默认值

            const serverRenderedCats = [];

            const existingEls = categoriesContainer.querySelectorAll('.news-category[data-cat-id]:not([data-cat-id="0"])');

            existingEls.forEach(function(el) {

                serverRenderedCats.push({ id: el.dataset.catId, name: el.textContent.trim() });
            ...
            } else if (serverRenderedCats.length > 0) {

                serverRenderedCats.forEach(function(cat) {
        ...
        // 渲染文章列表到页面（分页版，每页10篇）
            ...
            var c = document.querySelector('.news-list-container');
            if (!c) return;
            if (!articles || articles.length === 0) {
                c.innerHTML = '<div class="news-empty"><p>该分类下暂无文章</p></div>';
                renderPagination(currentPage, 0);
                return;
            }
            var h = '';
            articles.forEach(function(a) { h += createNewsCard(a); });
            c.innerHTML = h;
            var tp = Math.ceil((totalArticles || 0) / 10);
            if (tp < 1) tp = 1;
            renderPagination(currentPage, tp);
        }
...
// 跳转到指定页
        ...
        function goToPage(page) {
            ...
            loadNewsFromServer(page).then(function(articles) {
        ...
        document.addEventListener('DOMContentLoaded', async function() {
            ...
            if (preRendered) {
                ...
                try {
                    ...
                    totalArticles = parseInt(prerenderData.totalCnt) || 100;
                    ...
                    if (prerenderData.totalPages > 1) {
                        ...
                        loadNewsFromServer(1).then(function(articles) {
            ...
            loadNewsFromServer(1).then(articles => {
