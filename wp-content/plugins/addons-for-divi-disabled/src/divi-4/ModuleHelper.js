...
export const _getCustomBgCss = (
	...
	if ('on' === use_background_color_gradient) {
		...
		let _bg_grad_radial_direction =
			props[opt_name + '_bg_color_gradient_direction_radial'] || 'center';
		let _bg_color_gradient_stops =
			props[opt_name + '_bg_color_gradient_stops'] &&
			props[opt_name + '_bg_color_gradient_stops'].split('|').join(',');
		let _bg_repeat = props[opt_name + '_bg_color_gradient_repeat'];

		_bg_gradient_type =
			'circular' === _bg_gradient_type ? 'radial' : _bg_gradient_type;

		_bg_gradient_direction =
			_bg_gradient_type === 'linear'
				? _bg_gradient_direction
				: `circle at ${_bg_grad_radial_direction}`;

		let _bg_gradient_css = `${
			'on' === _bg_repeat ? 'repeating-' : ''
		}${_bg_gradient_type}-gradient( ${_bg_gradient_direction}, ${_bg_color_gradient_stops} )`;
	...
	if (_hover_enabled) {
		...
		if (
			...
			let _bg_gradient_direction_hover =
				...
				'180deg';
			let _bg_grad_radial_direction_hover =
				props[
					opt_name + '_bg_color_gradient_direction_radial__hover'
				] || 'center';
			let bg_color_gradient_stops_hover =
				props[opt_name + '_bg_color_gradient_stops__hover'] &&
				props[opt_name + '_bg_color_gradient_stops__hover']
					.split('|')
					.join(',');
			let _repeat_hover =
				props[opt_name + '_bg_color_gradient_repeat__hover'];

			_bg_gradient_type_hover =
				'circular' === _bg_gradient_type_hover
					? 'radial'
					: _bg_gradient_type_hover;
			...
			_bg_gradient_direction_hover =
				_bg_gradient_type_hover === 'linear'
					? _bg_gradient_direction_hover
					: `circle at ${_bg_grad_radial_direction_hover}`;

			let _bg_gradient_css_hover = `${
				'on' === _repeat_hover ? 'repeating-' : ''
			}${_bg_gradient_type_hover}-gradient( ${_bg_gradient_direction_hover}, ${bg_color_gradient_stops_hover} )`;
	...
	// alongside nav type
		...
		if (nav_x_center && nav_x_center === 'on') {
			...
			additionalCss.push([
				{
					selector: '%%order_class%% .dtq-carousel .slick-next',
					declaration: `right: calc(50% - ${
						parseInt(nav_width) + parseInt(nav_gap) / 2
					}px);`,
				},
			]);
			...
			additionalCss.push([
				{
					...
					device: 'tablet',
					declaration: `right: calc(50% - ${
						parseInt(nav_width_tablet) +
						parseInt(nav_gap_tablet) / 2
					}px);`,
				},
			]);
			...
			additionalCss.push([
				{
					...
					device: 'phone',
					declaration: `right: calc(50% - ${
						parseInt(nav_width_phone) + parseInt(nav_gap_phone) / 2
					}px);`,
				},
			]);
			...
			additionalCss.push([
				{
					...
					declaration: `margin-left: -${slide_spacing}; margin-right: -${slide_spacing};`,
				},
			]);
		}
	} else {
		additionalCss.push([
			{
				selector:
					'%%order_class%% .dtq-carousel .slick-slide, .et-db #et-boc %%order_class%% .dtq-carousel .slick-slide',
				declaration: `padding-top: ${slide_spacing}; padding-bottom: ${slide_spacing};`,
			},
		]);
			...
			additionalCss.push([
				{
					selector:
						'%%order_class%% .dtq-carousel .slick-list, .et-db #et-boc %%order_class%% .dtq-carousel .slick-list',
					declaration: `margin-top: -${slide_spacing}; margin-bottom: -${slide_spacing};`,
				},
			]);
		}
	}
...
export const get_pattern = (name, color, weight) => {
	let pattern = {
		curved: `data:image/svg+xml,%3Csvg xmlns='http://www.w3.org/2000/svg' preserveAspectRatio='none' overflow='visible' height='100%' viewBox='0 0 24 24' stroke='${color}' strokeWidth='${weight}' fill='none' stroke-linecap='square' stroke-miterlimit='10'%3E%3Cpath d='M0,6c6,0,6,13,12,13S18,6,24,6'/%3E%3C/svg%3E`,

		zigzag: `data:image/svg+xml,%3Csvg xmlns='http://www.w3.org/2000/svg' preserveAspectRatio='none' overflow='visible' height='100%' viewBox='0 0 24 24' stroke='${color}' strokeWidth='${weight}' fill='none' stroke-linecap='square' stroke-miterlimit='10'%3E%3Cpolyline points='0,18 12,6 24,18 '/%3E%3C/svg%3E`,

		square: `data:image/svg+xml,%3Csvg xmlns='http://www.w3.org/2000/svg' preserveAspectRatio='none' overflow='visible' height='100%' viewBox='0 0 24 24' fill='none' stroke='${color}' strokeWidth='${weight}' stroke-linecap='square' stroke-miterlimit='10'%3E%3Cpolyline points='0,6 6,6 6,18 18,18 18,6 24,6 '/%3E%3C/svg%3E`,

		curly: `data:image/svg+xml,%3Csvg xmlns='http://www.w3.org/2000/svg' preserveAspectRatio='none' overflow='visible' height='100%' viewBox='0 0 24 24' fill='none' stroke='${color}' strokeWidth='${weight}' stroke-linecap='square' stroke-miterlimit='10'%3E%3Cpath d='M0,21c3.3,0,8.3-0.9,15.7-7.1c6.6-5.4,4.4-9.3,2.4-10.3c-3.4-1.8-7.7,1.3-7.3,8.8C11.2,20,17.1,21,24,21'/%3E%3C/svg%3E`,
	};
