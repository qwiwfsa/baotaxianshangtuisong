...
class Metabox
{
	...
	public function save_popup_metabox($post_id, $post)
	{
		...
		if (
			!isset($_POST['_ba_display_conditions_nonce']) ||
			!wp_verify_nonce($_POST['_ba_display_conditions_nonce'], '_ba_display_conditions_nonce')
		) { //phpcs:ignore
			return;
		}
			...
			<div class="row">
				...
				<div class="dtq-popup-select">
					<select id="dtq-target" class="dtq-taget dtq-select2-initialized" name="conditions[target][]">
						...
						<!-- <option value="post_selected"><?php // esc_html_e( 'Posts', 'addons-for-divi' ); 
															?></option>
						<option value="is_front_page"><?php // esc_html_e( 'Frontpage', 'addons-for-divi' ); 
														?></option>
						<option value="is_404"><?php // esc_html_e( '404 Page', 'addons-for-divi' ); 
												?></option> -->
					</select>
				</div>
