...
class Renewal_Notice extends Notice_Base {
	 ...
	 * @var string
	 */
	public string $id = self::RENEWAL_NOTICE_SLUG;
	...
	/**
	 * @var string Placeholder for parent constructor; actual content from content().
	 */
	...
	/**
	 * Inner HTML for the renewal notice (icon + title + description + CTA).
	 */
	public function content(): string {
		if ( null === $this->days_diff ) {
			return ' ';
		}
	...
	/**
	 * Custom wrapper for renewal notice with dismiss attributes.
	 */
	public function render() {
		...
		printf(
			...
			esc_attr( wp_create_nonce( $this->get_action_name() ) ),
			$this->content() // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
		);
	}
	...
	public function is_dismissed(): bool {
		...
		$dismissed_at = get_user_meta( $user_id, self::DISMISSED_AT_USER_META, true );
	...
	public function maybe_set_conditions() {
		...
		if ( isset( $info->plan->features->retention ) && 'None' !== $info->plan->features->retention ) {
			return;
		}
		...
		$this->days_diff  = $this->date_diff_from_current( $info->plan->next_cycle_date );
	}

	/**
	 * Enqueue notice styles on screens where the renewal notice may appear.
	 * Must run on admin_enqueue_scripts so the stylesheet is output in the page head.
	 */
	...
	public function __construct() {
		add_action( 'current_screen', [ $this, 'maybe_set_conditions' ] );
