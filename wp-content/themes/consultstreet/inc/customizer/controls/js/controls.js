	...
	control: {
		...
		'consultstreet-color': {
			...
			init: function ( control ) {
				...
				// Init the control.
				consultstreet.input.color.init( control );
			},
			...
			template: function ( control ) {
				control.container.html(
					consultstreet.input.color.getTemplate( {
		...
		color: {
			...
			getTemplate: function ( data ) {
				...
				html +=
					...
					'" value="' +
					data.value +
					'" class="consultstreet-color-control" data-id="' +
					data['data-id'] +
					'"/>';

				return (
					'<div class="consultstreet-input-container" data-id="' +
					data.id +
					'">' +
					html +
					'</div>'
			...
			init: function ( control ) {
				var picker = jQuery(
					'.consultstreet-color-control[data-id="' + control.id + '"]'
				    ),
				    clear;
				...
				setTimeout( function () {
					clear = jQuery(
						'.consultstreet-input-container[data-id="' +
						control.id +
						'"] .wp-picker-clear'
					);
					if ( clear.length ) {
...
(
	function () {
		 ...
		 */
		wp.customize.consultstreetDynamicControl = wp.customize.Control.extend( {
			initialize: function ( id, options ) {
				...
				if ( !args.params.type ) {
					args.params.type = 'consultstreet-generic';
				}
			...
			ready: function () {
				...
				control.deferred.embedded.done( function () {
					control.initConsultStreetControl( control );
				} );
			},
			...
			embed: function () {
				...
				wp.customize.section( sectionId, function ( section ) {
					if (
						'consultstreet-expanded' === section.params.type ||
						section.expanded() ||
			 ...
			 * @returns {void}
			 */
			initConsultStreetControl: function ( control ) {
				if ( 'undefined' !== typeof consultstreet.control[control.params.type] ) {
					consultstreet.control[control.params.type].init( control );
					return;
				}
			...
			consultstreetValidateCSSValue: function ( value ) {
				var validUnits = [
					    'rem',
					    'em',
					    'ex',
...
_.each( consultstreet.control, function ( obj, type ) {
 ...
 * Control: Slider.
 */
wp.customize.controlConstructor['consultstreet-slider'] = wp.customize.Control.extend( {
	...
	ready: function () {
		...
		var control      = this,
		    ...
		    slider       = control.container.find( '.consultstreet-slider' ),
		...
		slider.slider( {
			...
			change: function ( event, ui ) {
				...
				// Save the values.
				control.initConsultStreetControl();
			}
		} );
		...
		// Init the control.
		if ( !_.isUndefined( window.consultstreetControlLoader ) && _.isFunction( consultstreetControlLoader ) ) {
			consultstreetControlLoader( control );
		} else {
			control.initConsultStreetControl();
		}
	},

	initConsultStreetControl: function () {
 ...
 * Control: Text.
 */
wp.customize.controlConstructor['consultstreet-text'] = wp.customize.Control.extend( {
 ...
 * Control: Sortable.
 */
/* global consultstreetControlLoader */
wp.customize.controlConstructor['consultstreet-sortable'] = wp.customize.Control.extend( {
	...
	ready: function () {
		...
		// Init the control.
		if (
			!_.isUndefined( window.consultstreetControlLoader ) &&
			_.isFunction( consultstreetControlLoader )
		) {
			consultstreetControlLoader( control );
		} else {
			control.initConsultStreetControl();
	...
	initConsultStreetControl: function () {
		...
		var control = this;
		control.container.find( '.consultstreet-controls-loading-spinner' ).hide();
 ...
 * Control: Radio image.
 */
wp.customize.controlConstructor['consultstreet-radio-image'] = wp.customize.consultstreetDynamicControl.extend( {} );

/**
 * Control: Toggle.
 */
wp.customize.controlConstructor['consultstreet-toggle'] = wp.customize.consultstreetDynamicControl.extend( {
 ...
 */
wp.customize.controlConstructor['consultstreet-radio-buttonset'] = wp.customize.Control.extend( {} );

/**
 * Control: Dimensions.
 */
;
/* global dimensionsconsultstreetL10n */
/* global consultstreetControlLoader */
wp.customize.controlConstructor['consultstreet-dimensions'] = wp.customize.consultstreetDynamicControl.extend( {
	...
	updateDimensionsValue: function ( context, value ) {
		...
		control.container.on( 'change keyup paste', '.' + context + ' input', function () {
			...
			// Notifications.
			control.consultstreetNotifications();
	 ...
	 */
	consultstreetNotifications: function () {
			...
			setting.bind( function ( value ) {
				...
				_.each( ['top', 'right', 'bottom', 'left'], function ( direction ) {
					if ( !_.isUndefined( value[direction] ) ) {
						if ( false === control.consultstreetValidateCSSValue( value[direction] ) ) {
							subs[direction] = dimensionsconsultstreetL10n[direction];
						} else {
				...
				if ( !_.isEmpty( subs ) ) {
					message = dimensionsconsultstreetL10n['invalid-value'] + ' (' + _.values( subs ).toString() + ') ';
...
;
/* global consultstreetControlLoader */
wp.customize.controlConstructor['consultstreet-background'] = wp.customize.Control.extend( {
	...
	initConsultStreetControl: function () {
		...
		var control = this,
		    value   = control.setting._value,
		    picker  = control.container.find( '.consultstreet-color-control' );
 ...
 * Control: Editor.
 */
wp.customize.controlConstructor['consultstreet-editor'] = wp.customize.consultstreetDynamicControl.extend( {
	...
	initConsultStreetControl: function () {
		...
		var id = 'consultstreet-editor-' + control_id;
