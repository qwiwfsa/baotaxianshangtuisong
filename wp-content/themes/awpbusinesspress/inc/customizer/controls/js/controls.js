	...
	control: {
		...
		'awpbusinesspress-color': {
			...
			init: function ( control ) {
				...
				// Init the control.
				awpbusinesspress.input.color.init( control );
			},
			...
			template: function ( control ) {
				control.container.html(
					awpbusinesspress.input.color.getTemplate(
						{
		...
		color: {
			...
			getTemplate: function ( data ) {
				...
				html +=
					...
					'" value="' +
					data.value +
					'" class="awpbusinesspress-color-control" data-id="' +
					data['data-id'] +
					'"/>';

				return (
					'<div class="awpbusinesspress-input-container" data-id="' +
					data.id +
					'">' +
					html +
					'</div>'
			...
			init: function ( control ) {
				var picker = jQuery(
					'.awpbusinesspress-color-control[data-id="' + control.id + '"]'
				),
					clear;
				...
				setTimeout(
					function () {
						clear = jQuery(
							'.awpbusinesspress-input-container[data-id="' +
							control.id +
							'"] .wp-picker-clear'
						);
						if ( clear.length ) {
...
(
	function () {
		 ...
		 */
		wp.customize.awpbusinesspressDynamicControl = wp.customize.Control.extend(
			{
				initialize: function ( id, options ) {
					...
					if ( ! args.params.type ) {
						args.params.type = 'awpbusinesspress-generic';
					}
				...
				ready: function () {
					...
					control.deferred.embedded.done(
						function () {
							control.initawpbusinesspressControl( control );
						}
					);
				},
				...
				embed: function () {
					...
					wp.customize.section(
						...
						function ( section ) {
							if (
								'awpbusinesspress-expanded' === section.params.type ||
								section.expanded() ||
				...
				focus: function ( args ) {
					 ...
					 * @returns {void}
					 */
				initawpbusinesspressControl: function ( control ) {
					if ( 'undefined' !== typeof awpbusinesspress.control[control.params.type] ) {
						awpbusinesspress.control[control.params.type].init( control );
						return;
					}
				...
				awpbusinesspressValidateCSSValue: function ( value ) {
					var validUnits = [
					'rem',
					'em',
					'ex',
...
_.each(
	awpbusinesspress.control,
	function ( obj, type ) {
		wp.customize.controlConstructor[
		type
		] = wp.customize.awpbusinesspressDynamicControl.extend( {} );
	}
);
 ...
 * Control: Slider.
 */
wp.customize.controlConstructor['awpbusinesspress-slider'] = wp.customize.Control.extend(
	{
		...
		ready: function () {
			...
			'postMessage' === control.setting.transport
				...
				slider       = control.container.find( '.awpbusinesspress-slider' ),
			...
			slider.slider(
				{
					...
					change: function ( event, ui ) {
						...
						// Save the values.
						control.initawpbusinesspressControl();
					}
					}
			);
			...
			// Init the control.
			if ( ! _.isUndefined( window.awpbusinesspressControlLoader ) && _.isFunction( awpbusinesspressControlLoader ) ) {
				awpbusinesspressControlLoader( control );
			} else {
				control.initawpbusinesspressControl();
			}
		},

		initawpbusinesspressControl: function () {
 ...
 * Control: Text.
 */
wp.customize.controlConstructor['awpbusinesspress-text'] = wp.customize.Control.extend(
	{
 ...
 * Control: Sortable.
 */
/* global awpbusinesspressControlLoader */
wp.customize.controlConstructor['awpbusinesspress-sortable'] = wp.customize.Control.extend(
	{
		...
		ready: function () {
			...
			// Init the control.
			if (
				! _.isUndefined( window.awpbusinesspressControlLoader ) &&
				_.isFunction( awpbusinesspressControlLoader )
			) {
				awpbusinesspressControlLoader( control );
			} else {
				control.initawpbusinesspressControl();
		...
		initawpbusinesspressControl: function () {
			...
			var control = this;
			control.container.find( '.awpbusinesspress-controls-loading-spinner' ).hide();
 ...
 * Control: Radio image.
 */
wp.customize.controlConstructor['awpbusinesspress-radio-image'] = wp.customize.awpbusinesspressDynamicControl.extend( {} );

/**
 * Control: Toggle.
 */
wp.customize.controlConstructor['awpbusinesspress-toggle'] = wp.customize.awpbusinesspressDynamicControl.extend(
	{
 ...
 */
wp.customize.controlConstructor['awpbusinesspress-radio-buttonset'] = wp.customize.Control.extend( {} );

/**
 * Control: Dimensions.
 */
;
/* global dimensionsawpbusinesspressL10n */
/* global awpbusinesspressControlLoader */
wp.customize.controlConstructor['awpbusinesspress-dimensions'] = wp.customize.awpbusinesspressDynamicControl.extend(
	{
		...
		updateDimensionsValue: function ( context, value ) {
			...
			control.container.on(
				...
				function () {
					...
					// Notifications.
					control.awpbusinesspressNotifications();
		...
		saveValue: function ( value ) {
			 ...
			 */
		awpbusinesspressNotifications: function () {
				...
				function ( setting ) {
					setting.bind(
						function ( value ) {
							...
							_.each(
								...
								function ( direction ) {
									if ( ! _.isUndefined( value[direction] ) ) {
										if ( false === control.awpbusinesspressValidateCSSValue( value[direction] ) ) {
											subs[direction] = dimensionsawpbusinesspressL10n[direction];
										} else {
							...
							if ( ! _.isEmpty( subs ) ) {
										message = dimensionsawpbusinesspressL10n['invalid-value'] + ' (' + _.values( subs ).toString() + ') ';
...
;
/* global awpbusinesspressControlLoader */
wp.customize.controlConstructor['awpbusinesspress-background'] = wp.customize.Control.extend(
	{
		...
		initawpbusinesspressControl: function () {
			...
			var control = this,
				value   = control.setting._value,
				picker  = control.container.find( '.awpbusinesspress-color-control' );
 ...
 * Control: Editor.
 */
wp.customize.controlConstructor['awpbusinesspress-editor'] = wp.customize.awpbusinesspressDynamicControl.extend(
	{
		...
		initawpbusinesspressControl: function () {
			...
			var id = 'awpbusinesspress-editor-' + control_id;
