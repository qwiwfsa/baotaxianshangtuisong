(function ($) {
	'use strict';
	wp.awpbusinesspress_customizerRepeater = {
		...
		init: function () {
			$( '.iconpicker-items>i' ).on(
				...
				function () {
					...
					awpbusinesspress_customizer_repeater_refresh_social_icons( th );
					return false;
				}
			);
		},
	...
	$( document ).ready(
		function () {
			wp.awpbusinesspress_customizerRepeater.init();

			$( '.iconpicker-search' ).on(
				'keyup',
				function () {
					wp.awpbusinesspress_customizerRepeater.search( $( this ) );
				}
			);
			...
			$( '.icp-auto' ).on(
				'click',
				function () {
					wp.awpbusinesspress_customizerRepeater.iconPickerToggle( $( this ) );
				}
			);
