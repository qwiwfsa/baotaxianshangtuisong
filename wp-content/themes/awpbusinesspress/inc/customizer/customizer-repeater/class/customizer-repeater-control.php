...
class awpbusinesspress_Repeater extends WP_Customize_Control {
	...
	public function __construct( $manager, $id, $args = array() ) {
		...
		$this->add_field_label = esc_html__( 'Add new field', 'awpbusinesspress' );
		if ( ! empty( $args['add_field_label'] ) ) {
			$this->add_field_label = $args['add_field_label'];
		}

		$this->boxtitle = esc_html__( 'Customizer Repeater', 'awpbusinesspress' );
	...
	public function enqueue() {
		...
		wp_enqueue_style( 'awpbusinesspress_customizer-repeater-admin-stylesheet', get_template_directory_uri() . '/inc/customizer/customizer-repeater/css/admin-style.css', array(), 999 );

		wp_enqueue_style( 'wp-color-picker' );

		wp_enqueue_script( 'awpbusinesspress_customizer-repeater-script', get_template_directory_uri() . '/inc/customizer/customizer-repeater/js/customizer_repeater.js', array( 'jquery', 'jquery-ui-draggable', 'wp-color-picker' ), 999, true );

		wp_enqueue_script( 'awpbusinesspress_customizer-repeater-fontawesome-iconpicker', get_template_directory_uri() . '/inc/customizer/customizer-repeater/js/fontawesome-iconpicker.js', array( 'jquery' ), 999, true );

		wp_enqueue_style( 'awpbusinesspress_customizer-repeater-fontawesome-iconpicker-script', get_template_directory_uri() . '/inc/customizer/customizer-repeater/css/fontawesome-iconpicker.min.css', array(), 999 );
	}
				...
				<div class="customizer-repeater-general-control-repeater-container customizer-repeater-draggable">
					...
					<div class="customizer-repeater-box-content-hidden">
						...
						if ( $this->customizer_repeater_title_control == true ) {
							$this->input_control(
								array(
									'label' => apply_filters( 'awpbusinesspress_repeater_input_labels_filter', esc_html__( 'Title', 'awpbusinesspress' ), $this->id, 'customizer_repeater_title_control' ),
									'class' => 'customizer-repeater-title-control',
									'type'  => apply_filters( 'awpbusinesspress_repeater_input_types_filter', '', $this->id, 'customizer_repeater_title_control' ),
								),
								$title
							);
						}
						if ( $this->customizer_repeater_subtitle_control == true ) {
							$this->input_control(
								array(
									'label' => apply_filters( 'awpbusinesspress_repeater_input_labels_filter', esc_html__( 'Subtitle', 'awpbusinesspress' ), $this->id, 'customizer_repeater_subtitle_control' ),
									'class' => 'customizer-repeater-subtitle-control',
									'type'  => apply_filters( 'awpbusinesspress_repeater_input_types_filter', '', $this->id, 'customizer_repeater_subtitle_control' ),
								),
								$subtitle
							);
						}
						if ( $this->customizer_repeater_text_control == true ) {
							$this->input_control(
								array(
									'label' => apply_filters( 'awpbusinesspress_repeater_input_labels_filter', esc_html__( 'Description', 'awpbusinesspress' ), $this->id, 'customizer_repeater_text_control' ),
									'class' => 'customizer-repeater-text-control',
									'type'  => apply_filters( 'awpbusinesspress_repeater_input_types_filter', 'textarea', $this->id, 'customizer_repeater_text_control' ),
								),
								$text
							);
						}
						...
						if ( $this->customizer_repeater_button_text_control ) {
							$this->input_control(
								array(
									'label' => apply_filters(
										'awpbusinesspress_repeater_input_labels_filter',
										esc_html__(
											'Button Text',
											'awpbusinesspress'
										),
									...
									'type'  => apply_filters(
										'awpbusinesspress_repeater_input_types_filter',
										'',
										$this->id,
										'customizer_repeater_button_text_control'
									),
						...
						if ( $this->customizer_repeater_link_control ) {
							$this->input_control(
								array(
									'label'             => apply_filters( 'awpbusinesspress_repeater_input_labels_filter', esc_html__( 'Link', 'awpbusinesspress' ), $this->id, 'customizer_repeater_link_control' ),
									'class'             => 'customizer-repeater-link-control',
									'sanitize_callback' => 'esc_url_raw',
									'type'              => apply_filters( 'awpbusinesspress_repeater_input_types_filter', '', $this->id, 'customizer_repeater_link_control' ),
								),
								$link
							);
						}
						...
						if ( $this->customizer_repeater_color_control == true ) {
							$this->input_control(
								array(
									'label'             => apply_filters( 'awpbusinesspress_repeater_input_labels_filter', esc_html__( 'Color', 'awpbusinesspress' ), $this->id, 'customizer_repeater_color_control' ),
									'class'             => 'customizer-repeater-color-control',
									'type'              => apply_filters( 'awpbusinesspress_repeater_input_types_filter', 'color', $this->id, 'customizer_repeater_color_control' ),
						...
						if ( $this->customizer_repeater_shortcode_control == true ) {
							$this->input_control(
								array(
									'label' => apply_filters( 'awpbusinesspress_repeater_input_labels_filter', esc_html__( 'Shortcode', 'awpbusinesspress' ), $this->id, 'customizer_repeater_shortcode_control' ),
									'class' => 'customizer-repeater-shortcode-control',
									'type'  => apply_filters( 'awpbusinesspress_repeater_input_types_filter', '', $this->id, 'customizer_repeater_shortcode_control' ),
								),
								$shortcode
							);
						}
						...
						if ( $this->customizer_repeater_designation_control == true ) {
							$this->input_control(
								array(
									'label' => apply_filters( 'awpbusinesspress_repeater_input_labels_filter', esc_html__( 'Designation', 'awpbusinesspress' ), $this->id, 'customizer_repeater_designation_control' ),
									'class' => 'customizer-repeater-designation-control',
									'type'  => apply_filters( 'awpbusinesspress_repeater_input_types_filter', 'textarea', $this->id, 'customizer_repeater_designation_control' ),
								),
								$designation
							);
						}
						...
						?>
						>
							<?php esc_html_e( 'Delete field', 'awpbusinesspress' ); ?>
						</button>
	...
	private function testimonila_check( $value = 'no' ) {
		...
		<?php esc_html_e( 'Open link in new tab', 'awpbusinesspress' ); ?>
		</span>
		<select class="customizer-repeater-checkbox">
			<option value="no" <?php selected( $value, 'no' ); ?>>
			<?php esc_html_e( 'No', 'awpbusinesspress' ); ?>
			</option>
			<option value="yes" <?php selected( $value, 'yes' ); ?>>
			<?php esc_html_e( 'Yes', 'awpbusinesspress' ); ?>
			</option>
		</select>
		<?php
	}
	...
	private function icon_picker_control( $value = '', $show = '' ) {
		...
		>
			<span class="customize-control-title">
				<?php esc_html_e( 'Icon', 'awpbusinesspress' ); ?>
			</span>
			<span class="description customize-control-description">
					<?php
					echo sprintf(
					/* translators: %1$s: icon */
						esc_html__( 'Note: Some icons may not be displayed here. You can see the full list of icons at %1$s.', 'awpbusinesspress' ),
						sprintf( '<a href="//fontawesome.io/icons/" rel="nofollow">%s</a>', esc_html__( '//fontawesome.io/icons/', 'awpbusinesspress' ) )
					);
					?>
			</span>
	...
	private function image_control( $value = '', $show = '' ) {
		...
		>
			<span class="customize-control-title">
				<?php esc_html_e( 'Image', 'awpbusinesspress' ); ?>
			</span>
			<input type="text" class="widefat custom-media-url" value="<?php echo esc_attr( $value ); ?>">
			<input type="button" class="button button-secondary customizer-repeater-custom-media-button" value="<?php esc_attr_e( 'Upload Image', 'awpbusinesspress' ); ?>" />
		</div>
		<?php
	}
	...
	private function icon_type_choice( $value = 'customizer_repeater_icon' ) {
		?>
		<span class="customize-control-title">
			<?php esc_html_e( 'Image type', 'awpbusinesspress' ); ?>
		</span>
		<select class="customizer-repeater-image-choice">
			<option value="customizer_repeater_icon" <?php selected( $value, 'customizer_repeater_icon' ); ?>><?php esc_html_e( 'Icon', 'awpbusinesspress' ); ?></option>
			<option value="customizer_repeater_image" <?php selected( $value, 'customizer_repeater_image' ); ?>><?php esc_html_e( 'Image', 'awpbusinesspress' ); ?></option>
			<option value="customizer_repeater_none" <?php selected( $value, 'customizer_repeater_none' ); ?>><?php esc_html_e( 'None', 'awpbusinesspress' ); ?></option>
		</select>
		<?php
	}
	...
	private function repeater_control( $value = '' ) {
		...
		$show_del        = 0;
		?>
		<span class="customize-control-title"><?php esc_html_e( 'Social icons', 'awpbusinesspress' ); ?></span>
		<?php
		if ( ! empty( $value ) ) {
			...
			<div class="customizer-repeater-social-repeater">
				<div class="customizer-repeater-social-repeater-container">
					...
					<input type="text" class="customizer-repeater-social-repeater-link"
						placeholder="<?php esc_attr_e( 'Link', 'awpbusinesspress' ); ?>">
					<input type="hidden" class="customizer-repeater-social-repeater-id" value="">
					<button class="social-repeater-remove-social-item" style="display:none">
						<?php esc_html_e( 'Remove Icon', 'awpbusinesspress' ); ?>
					</button>
				</div>
				<input type="hidden" id="social-repeater-socials-repeater-colector" class="social-repeater-socials-repeater-colector" value=""/>
			</div>
			<button class="social-repeater-add-social-item button-secondary"><?php esc_html_e( 'Add Icon', 'awpbusinesspress' ); ?></button>
			<?php
		} else {
			?>
			<div class="customizer-repeater-social-repeater">
				...
				foreach ( $social_repeater as $social_icon ) {
					...
					<div class="customizer-repeater-social-repeater-container">
						...
						<button class="social-repeater-remove-social-item"
								...
								?>
								"><?php esc_html_e( 'Remove Icon', 'awpbusinesspress' ); ?></button>
					</div>
					<?php
				}
				?>
