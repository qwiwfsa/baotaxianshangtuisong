<?php
if ( ! function_exists( 'awpbusinesspress_excerpt_length' ) ) :
	/**
	 ...
	 */
	function awpbusinesspress_excerpt_length( $length ) {
		if ( is_admin() ) {
			return $length;
		}

		// Getting data from Customizer Options.
		$length = get_theme_mod( 'awpbusinesspress_excerpt_length', 30 );

		return absint( $length );
	}
	endif; // awpbusinesspress_excerpt_length.
	add_filter( 'excerpt_length', 'awpbusinesspress_excerpt_length', 999 );

if ( ! function_exists( 'awpbusinesspress_excerpt_more' ) ) :
	/**
	 * Replaces "[...]" (appended to automatically generated excerpts) with ... and a option from customizer
	 *
	 * @return string option from customizer prepended with an ellipsis.
	 */
	function awpbusinesspress_excerpt_more( $more ) {
		if ( is_admin() ) {
			return $more;
		}
