 ...
 */
function elmira_theme_setup(){
    /*
     * Make child theme available for translation.
     * Translations can be filed in the /languages/ directory.
     */
    load_child_theme_textdomain( 'elmira', get_stylesheet_directory() . '/languages' );	
	require get_stylesheet_directory() . '/inc/customizer/elmira-customizer-options.php';	
}
add_action( 'after_setup_theme', 'elmira_theme_setup' );

/**
 * Load assets.
 */

function elmira_theme_css() {
	wp_enqueue_style( 'elmira-parent-style', get_template_directory_uri() . '/style.css' );
	wp_enqueue_style('elmira-child-style', get_stylesheet_directory_uri() . '/style.css');
	wp_enqueue_style('elmira-default-css', get_stylesheet_directory_uri() . "/assets/css/theme-default.css" );
    wp_enqueue_style('elmira-bootstrap-smartmenus-css', get_stylesheet_directory_uri() . "/assets/css/bootstrap-smartmenus.css" ); 	
}
add_action( 'wp_enqueue_scripts', 'elmira_theme_css', 99);

/**
 * Import Options From Parent Theme
 *
 */
function elmira_parent_theme_options() {
...
add_action( 'after_switch_theme', 'elmira_parent_theme_options' );

/**
 * Fresh site activate
 *
 */
$fresh_site_activate = get_option( 'fresh_elmira_site_activate' );
if ( (bool) $fresh_site_activate === false ) {
	...
	update_option( 'fresh_elmira_site_activate', true );
}

/**
 * Custom Theme Script
*/
function elmira_custom_theme_css() {
	$elmira_testomonial_background_image = get_theme_mod('arilewp_testomonial_background_image');
	?>
    <style type="text/css">
		<?php if($elmira_testomonial_background_image != null) : ?>
		.theme-testimonial { 
		        background-image: url(<?php echo esc_url( $elmira_testomonial_background_image ); ?>); 
...
<?php }
add_action('wp_footer','elmira_custom_theme_css');

/**
 * Page header
 *
 */
function elmira_custom_header_setup() {
	add_theme_support( 'custom-header', apply_filters( 'elmira_custom_header_args', array(
		...
		'flex-height'        => true,
		'flex-width'         => true,
		'wp-head-callback'   => 'elmira_header_style',
	) ) );
}

add_action( 'after_setup_theme', 'elmira_custom_header_setup' );

/**
 * Custom background
 *
 */
function elmira_custom_background_setup() {
	add_theme_support( 'custom-background', apply_filters( 'elmira_custom_background_args', array(
		'default-color' => '202020',
		'default-image' => '',
	) ) );
}
add_action( 'after_setup_theme', 'elmira_custom_background_setup' );


if ( ! function_exists( 'elmira_header_style' ) ) :
	/**
	 * Styles the header image and text displayed on the blog.
	 *
	 * @see elmira_custom_header_setup().
	 */
	function elmira_header_style() {
