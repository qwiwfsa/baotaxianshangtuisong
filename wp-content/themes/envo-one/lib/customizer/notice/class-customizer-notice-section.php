<?php
/**
 * Envo One Customizer Notification Section Class.
 *
 * @package Envo One
 */

/**
 * Envo_One_Customizer_Notice_Section class
 */
class Envo_One_Customizer_Notice_Section extends WP_Customize_Section {
	/**
	...
	public function json() {
		$json = parent::json();
		global $envo_one_recommended_plugins;
		...
		$show_recommended_plugins = get_option( 'envo_one_show_recommended_plugins' );

		foreach ( $envo_one_recommended_plugins as $slug => $plugin_opt ) {
			...
			$envo_one_recommended_plugin['url']    = $this->create_action_link( $active['needs'], $slug );
			if ( $active['needs'] !== 'install' && $active['status'] ) {
				$envo_one_recommended_plugin['class'] = 'active';
			} else {
				$envo_one_recommended_plugin['class'] = '';
			}

			switch ( $active['needs'] ) {
				case 'install':
					$envo_one_recommended_plugin['button_class'] = 'install-now button';
					$envo_one_recommended_plugin['button_label'] = $install_button_label;
					break;
				case 'activate':
					$envo_one_recommended_plugin['button_class'] = 'activate-now button button-primary';
					$envo_one_recommended_plugin['button_label'] = $activate_button_label;
					break;
				case 'deactivate':
					$envo_one_recommended_plugin['button_class'] = 'deactivate-now button';
					$envo_one_recommended_plugin['button_label'] = $deactivate_button_label;
					break;
			}
			$info   = $this->call_plugin_api( $slug );
			$envo_one_recommended_plugin['id'] = $slug;
			$envo_one_recommended_plugin['plugin_slug'] = $slug;

			if ( ! empty( $plugin_opt['description'] ) ) {
				$envo_one_recommended_plugin['description'] = $plugin_opt['description'];
			} else {
				$envo_one_recommended_plugin['description'] = $info->short_description;
			}

			$envo_one_recommended_plugin['title'] = $info->name;

			$customize_plugins[] = $envo_one_recommended_plugin;
				...
				<div class="recomended-actions_container" id="plugin-filter">
					<# if( data.recommended_plugins.length > 0 ){ #>
						<# for (action in data.recommended_plugins) { #>
							<div class="envo-one-recommeded-actions-container envo-one-recommended-plugins" data-index="{{ data.recommended_plugins[action].index }}">
								<# if( !data.recommended_plugins[action].check ){ #>
									<div class="envo-one-recommeded-actions">
