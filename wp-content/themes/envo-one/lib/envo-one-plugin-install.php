...
if ( ! class_exists( 'Envo_One_Plugin_Install' ) ) :
	/**
	 * The Envo One plugin install class
	 */
	class Envo_One_Plugin_Install {
		...
		public static function install_plugin_button( $plugin_slug, $plugin_file, $plugin_name, $classes = array(), $activated = '', $activate = '', $install = '' ) {
			if ( current_user_can( 'install_plugins' ) && current_user_can( 'activate_plugins' ) ) {
				if ( is_plugin_active( $plugin_slug . '/' . $plugin_file ) ) {
					...
					$button = array(
						'message' => esc_attr__( 'Activated', 'envo-one' ),
						'url'     => '#',
						'classes' => array( 'envo-one-button', 'disabled' ),
					);
					...
					$button = array(
						'message' => esc_attr__( 'Activate', 'envo-one' ),
						'url'     => $url,
						'classes' => array( 'envo-one-button', 'activate-now' ),
					);
					...
					$button = array(
						'message' => esc_attr__( 'Install now', 'envo-one' ),
						'url'     => $url,
						'classes' => array( 'envo-one-button', 'envo-one-install-now', 'install-now', 'install-' . $plugin_slug ),
					);
				...
				?>
				<span class="envo-one-plugin-card plugin-card-<?php echo esc_attr( $plugin_slug ); ?>">
				...
				<a href="https://wordpress.org/plugins/<?php echo esc_attr( $plugin_slug ); ?>" target="_blank"><?php esc_html_e( 'Learn more', 'envo-one' ); ?></a>
				<?php
			}
		}
...
return new Envo_One_Plugin_Install();
