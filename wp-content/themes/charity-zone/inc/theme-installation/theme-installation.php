<?php
/**
 * Charity_Zone - Theme Info Admin Menu
 * @package TheMagnifico52
 * @subpackage Admin
 */
if ( ! class_exists( 'charity_zone_Theme_Info' ) ) {
    class charity_zone_Theme_Info{
        ...
        public function __construct( $config ) {
            $this->config = $config;
            $this->charity_zone_charity_zone_prepare_class();

            /*admin menu*/
            add_action( 'admin_menu', array( $this, 'charity_zone_tm_admin_menu' ) );

            /* enqueue script and style for about page */
            add_action( 'admin_enqueue_scripts', array( $this, 'charity_zone_style_and_scripts' ) );

            /* ajax callback for dismissable required actions */
            add_action( 'wp_ajax_tm_theme_info_update_recommend_action', array( $this, 'charity_zone_update_recommended_action_callback' ) );
        }

        /**
         * Prepare and setup class properties.
         */
        public function charity_zone_charity_zone_prepare_class() {
            $theme = wp_get_theme();
            $this->theme_name    = esc_html( $theme->get( 'Name' ) );
            $this->theme_slug    = $theme->get_template();
            $this->theme_version = $theme->get( 'Version' );
            $this->page_title    = $this->theme_name . esc_html__( ' Info', 'charity-zone' );
            $this->menu_title    = $this->theme_name . esc_html__( ' Theme', 'charity-zone' );
         ...
         */
        public function charity_zone_update_recommended_action_callback() {
        ...
        private function charity_zone_get_recommended_actions() {
            $saved_actions = get_option( $this->theme_slug . '_recommended_actions' );
        ...
        private function charity_zone_count_recommended_actions() {
            $count = 0;
            $actions_count = $this->charity_zone_get_recommended_actions();
         ...
         */
        function charity_zone_tm_admin_menu() {

            if ( ! empty( $this->page_title ) && ! empty( $this->menu_title ) ) {
                $count = $this->charity_zone_count_recommended_actions();
                ...
                add_theme_page( $this->page_title, $menu_title, 'edit_theme_options', $this->theme_slug . '-info', array(
                    $this,
                    'charity_zone_tm_theme_info_screen_main',
                ) );
            }
        }
         ...
         */
        public function charity_zone_tm_theme_info_screen_main() {
            ...
            if (
                ...
                /* Display tabs */
                if ( ! empty( $this->tabs ) ) {
                    $current_tab = isset( $_GET['tab'] ) ? wp_unslash( $_GET['tab'] ) : 'charity_zone_demo_impoter';
         ...
         * Getting started tab
         */
        public function charity_zone_demo_impoter() {
            echo '<div class="feature-section demo-impoter-tab ">';
            if ( ! empty( $this->config['charity_zone_demo_impoter_steps'] ) ) {
                $charity_zone_demo_impoter_steps = $this->config['charity_zone_demo_impoter_steps'];
                if ( ! empty( $charity_zone_demo_impoter_steps ) ) { ?>
                        <?php
                            ...
                            <div class="impoter-box-main">
                                <div class="wrap-main-box">
                                    <div class="main-box">
                                        <h2><?php echo esc_html('Welcome to Charity_Zone','charity-zone'); ?></h2>
                            ...
                            <div class="impoter-left">
                                <div class="importer-description">
                                    <div class="importer-buttons">
                                        <a target="_blank" href="<?php echo esc_url( CHARITY_ZONE_GET_PREMIUM_PRO ); ?>" class="btn btn-purple"><?php echo esc_html('Get Premium','charity-zone'); ?></a>
                                        <a target="_blank" href="<?php echo esc_url( CHARITY_ZONE_LIVE_DEMO ); ?>" class="btn btn-green"><?php echo esc_html('View Demo','charity-zone'); ?></a>
                                        <a target="_blank" href="<?php echo esc_url( CHARITY_ZONE_FREE_DOC ); ?>" class="btn btn-yellow"><?php echo esc_html('Lite Doc','charity-zone'); ?></a>
                                        <a target="_blank" href="<?php echo esc_url( CHARITY_ZONE_CONTACT_SUPPORT ); ?>" class="btn btn-blue"><?php echo esc_html('Lite Support','charity-zone'); ?></a>
                                    </div>
                                </div>
         ...
         * Getting started tab
         */
        public function charity_zone_pro_templates() {
            echo '<div class="feature-section demo-impoter-tab ">'; ?>
         ...
         */
        public function charity_zone_check_plugin_status( $slug ) {
        ...
        public function charity_zone_create_action_link( $state, $slug ) {
        ...
        public function recommended_actions() {
            ...
            if ( ! empty( $recommended_actions ) ) {
                ...
                if ( ! empty( $recommended_actions ) && is_array( $recommended_actions ) ) {
                    ...
                    /*defaults values for charity_zone_getting_started array */
                    $defaults = array(
                        'title'         => '',
                        'desc'          => '',
                        'check'         => false,
                    ...
                    foreach ( $recommended_actions as $action_key => $action_value ) {
                        ...
                        if ( ! empty( $plugin_slug ) ) {
                            ...
                            $active = $this->charity_zone_check_plugin_status( $action_value['plugin_slug'] );
                            $url    = $this->charity_zone_create_action_link( $active['needs'], $action_value['plugin_slug'] );
                            $label  = '';
                            $class  = '';
                            switch ( $active['needs'] ) {
                                ...
                                case 'install':
                                    ...
                                    $label = esc_html__( 'Install', 'charity-zone' );
                                    break;

                                case 'activate':
                                    $class = 'activate-now button button-primary';
                                    $label = esc_html__( 'Activate', 'charity-zone' );

                                    break;

                                case 'deactivate':
                                    $class = 'deactivate-now button';
                                    $label = esc_html__( 'Deactivate', 'charity-zone' );
                ...
                if ( $hooray ){
                    echo '<span class="hooray">' . esc_html__( 'Hooray! There are no recommended actions for you right now.', 'charity-zone' ) . '</span>';
                    echo '<a data-action="reset" id="reset" class="reset-all" href="#">'.esc_html__('Show All Recommended Actions', 'charity-zone').'</a>';
                }
                echo '</div>';
            }
        }
         ...
         * Free vs Pro tab
         */
        public function charity_zone_free_pro() {
            $theme_name_config = esc_attr ( wp_get_theme()->get('Name') );
            $charity_zone_free_pro = isset( $this->config['charity_zone_free_pro'] ) ? $this->config['charity_zone_free_pro'] : array();
            if ( ! empty( $charity_zone_free_pro ) ) {
                ...
                if ( ! empty( $charity_zone_free_pro ) && is_array( $charity_zone_free_pro ) ) {
                    echo '<div class="feature-section">';
                    echo '<div id="charity_zone_free_pro" class="charity-zone-theme-installation-tab-pane charity-zone-theme-installation-fre-pro">';
                    echo '<table class="free-pro-table table-light-wrapper charity-zone-theme-installation-text-center">';
                    echo '<thead>';
                    echo '<tr>';
                    echo '<th>' . esc_html__( 'Theme Feature','charity-zone' ) . '</th>';
                    echo '<th>' . esc_html__( 'Basic Version','charity-zone' ) . '</th>';
                    echo '<th>' . esc_html__( 'Premium Version','charity-zone' ) . '</th>';
                    echo '</tr>';
                    echo '</thead>';
                    echo '<tbody>';
                    foreach ( $charity_zone_free_pro as $feature ) {
                    ...
                    echo '<td></td>';
                    echo '<td></td>';
                    echo '<td><a href="' . esc_url( CHARITY_ZONE_GET_PREMIUM_PRO ).'" target="_blank" class="button button-primary button-hero compare-pro">Buy Pro</a></td>';
                    echo '</tr>';
                    echo '</tbody>';
                    echo '</table>';
                    echo '</div>';
        ...
        public function charity_zone_recommended_plugins() {
            $charity_zone_recommended_plugins = $this->config['charity_zone_recommended_plugins'];

            if ( ! empty( $charity_zone_recommended_plugins ) ) {
                if ( ! empty( $charity_zone_recommended_plugins ) && is_array( $charity_zone_recommended_plugins ) ) {

                    echo '<div class="feature-section recommended-plugins col-wrap demo-import-boxed" id="plugin-filter">';

                    foreach ( $charity_zone_recommended_plugins as $charity_zone_recommended_plugins_item ) {

                        if ( ! empty( $charity_zone_recommended_plugins_item['slug'] ) ) {
                            $info   = $this->call_plugin_api( $charity_zone_recommended_plugins_item['slug'] );
                            if ( ! empty( $info->icons ) ) {
                                $icon = $this->get_plugin_icon( $info->icons );
                            }

                            $active = $this->charity_zone_check_plugin_status( $charity_zone_recommended_plugins_item['slug'] );

                            if ( ! empty( $active['needs'] ) ) {
                                $url = $this->charity_zone_create_action_link( $active['needs'], $charity_zone_recommended_plugins_item['slug'] );
                            }
                            ...
                            if ( ! empty(  $info->version ) ) {
                                echo '<span class="version">'. ( ! empty( $this->config['charity_zone_recommended_plugins']['version_label'] ) ? esc_html( $this->config['charity_zone_recommended_plugins']['version_label'] ) : '' ) . esc_html( $info->version ).'</span>';
                            }
                            ...
                            if ( ! empty( $info->name ) && ! empty( $active ) ) {
                                ...
                                echo '<span class="plugin-card-' . esc_attr( $charity_zone_recommended_plugins_item['slug'] ) . ' action_button ' . ( ( $active['needs'] !== 'install' && $active['status'] ) ? 'active' : '' ) . '">';
                                echo '<a data-slug="' . esc_attr( $charity_zone_recommended_plugins_item['slug'] ) . '" class="' . esc_attr( $class ) . '" href="' . esc_url( $url ) . '">' . esc_html( $label ) . '</a>';
                                echo '</span>';
                            }
         ...
         * Changelog tab
         */
        private function charity_zone_parse_changelog() {
            WP_Filesystem();
        ...
        public function charity_zone_changelog() {
            $charity_zone_changelog = $this->charity_zone_parse_changelog();
            if ( ! empty( $charity_zone_changelog ) ) {
                echo '<div class="featured-section changelog">';
                echo "<pre class='changelog'>";
                echo esc_html($charity_zone_changelog);
                echo "</pre>";
         ...
         */
        public function charity_zone_style_and_scripts( $hook_suffix ) {

            // this is needed on all admin pages, not just the about page, for the badge action count in the WordPress main sidebar
            wp_enqueue_style( 'charity-zone-theme-installation-css', get_template_directory_uri() . '/inc/theme-installation/assets/css/theme-installation.css' );

            if ( 'appearance_page_' . $this->theme_slug . '-info' == $hook_suffix ) {

                wp_enqueue_script( 'charity-zone-theme-installation-js', get_template_directory_uri() . '/inc/theme-installation/assets/js/theme-installation.js', array( 'jquery' ) );
                ...
                $count = $this->charity_zone_count_recommended_actions();
                wp_localize_script( 'charity-zone-theme-installation-js', 'tm_theme_info_box_object', array(
...
$config = array(
    ...
    'info_title' => sprintf( esc_html__( 'Welcome to %s - ', 'charity-zone' ), $theme_name_config ),

    // Main welcome content
    /* translators: %s - Theme Name*/
    'info_content' => sprintf( esc_html__( '%s is now installed and ready to use. We hope the following information will help and you enjoy using it!', 'charity-zone' ), '<b>'.$theme_name_config.'</b>' ),
    ...
    'quick_links' => array(
        'theme_url'  => array(
            'text' => __( 'Try the Demo', 'charity-zone' ),
            'url' => esc_url( CHARITY_ZONE_LIVE_DEMO )
        ),
        'pro_url'  => array(
            'text' => __( 'Get Premium', 'charity-zone' ),
            'url'  => esc_url( CHARITY_ZONE_GET_PREMIUM_PRO )
        ),
        'bundle_url'  => array(
            'text' => __( 'Buy All Themes - 120+ Templates', 'charity-zone' ),
            'url'  => esc_url( CHARITY_ZONE_BUNDLE_LINK )
        ),
    ),

    'tabs' => array(
        'charity_zone_demo_impoter'         => esc_html__( 'Demo Impoter', 'charity-zone' ),
        'charity_zone_pro_templates'        => esc_html__( 'Pro Templates', 'charity-zone' ),
        'charity_zone_free_pro'             => esc_html__( 'Compare Free Vs Pro', 'charity-zone' ),
        'charity_zone_recommended_plugins'  => esc_html__( 'Recommended Plugins', 'charity-zone' ),
        'charity_zone_changelog'            => esc_html__( 'Changelog', 'charity-zone' ),
    ),

    /*Getting started tab*/
    'charity_zone_gs_steps' => array(
        'first' => array(
            'title' => esc_html__( 'Checkout Premium', 'charity-zone' ),
            'desc' => esc_html__( 'Our premium theme comes with extended features like demo content import , responsive layouts etc.', 'charity-zone' ),
            'button_label' => esc_html__( 'Get Premium', 'charity-zone' ),
            'button_link' => esc_url( CHARITY_ZONE_GET_PREMIUM_PRO ),
            'is_button' => true,
            'is_new_tab' => true
        ),
        'second' => array (
            'title' => esc_html__( 'Contact Support', 'charity-zone' ),
            'desc' => esc_html__( 'Thank you for trying Charity_Zone , feel free to contact us for any support regarding our theme.', 'charity-zone' ),
            'button_label' => esc_html__( 'Contact Support', 'charity-zone' ),
            'button_link' => esc_url( CHARITY_ZONE_CONTACT_SUPPORT ),
            'is_button' => true,
            'is_new_tab' => true
        ),
        'third' => array (
            'title' => esc_html__( 'Review', 'charity-zone' ),
            'desc' => esc_html__( 'If You love Charity_Zone theme then we would appreciate your review about our theme.', 'charity-zone' ),
            'button_label' => esc_html__( 'Review', 'charity-zone' ),
            'button_link' => esc_url( CHARITY_ZONE_REVIEW ),
            'is_button' => true,
            'is_new_tab' => true
        ),
        'fourth' => array (
            'title' => esc_html__( 'Go to Customizer', 'charity-zone' ),
            'desc' => esc_html__( 'All Settings, Header & Footer Options and Theme Options are available via Customize screen.', 'charity-zone' ),
            'button_label' => esc_html__( 'Go to Customizer', 'charity-zone' ),
            'button_link' => esc_url( admin_url( 'customize.php' ) ),
            'is_button' => true,
            'is_new_tab' => true
        ),
        'fifth' => array(
            'title' => esc_html__( 'Free Documentation', 'charity-zone' ),
            'desc' => esc_html__( 'Our guide is available if you require any help configuring and setting up the theme. Easy and quick way to setup the theme.', 'charity-zone' ),
            'button_label' => esc_html__( 'Free Documentation', 'charity-zone' ),
            'button_link' => esc_url( CHARITY_ZONE_FREE_DOC ),
            'is_button' => true,
            'is_new_tab' => true
        ),
        'sixth' => array(
            'title' => esc_html__( 'View Theme Demo', 'charity-zone' ),
            'desc' => esc_html__( 'Explore the demo to see how easily you can set up and customize the theme.', 'charity-zone' ),
            'button_label' => esc_html__( 'View Demo', 'charity-zone' ),
    ...
    /*Getting started tab*/
    'charity_zone_demo_impoter_steps' => array(
        'first' => array(
            'title' => esc_html__( 'Instant Demo Setup', 'charity-zone' ),
            'desc' => esc_html__( 'Import your entire demo content in just one click, including pages, posts, and design elements for a quick setup.', 'charity-zone' ),
            'button_label' => esc_html__( 'Start Demo Import', 'charity-zone' ),
    ...
    'charity_zone_recommended_actions' => array(

    ),

    // Free vs pro array.
    'charity_zone_free_pro' => array(
        array(
            'desc'=> __( 'Header Background Color', 'charity-zone' ),
            'free' => __( 'yes','charity-zone' ),
            'pro'  => __( 'yes','charity-zone' ),
        ),
        array(
            'desc'=> __( 'Custom Navigation Logo Or Text', 'charity-zone' ),
            'free' => __( 'yes','charity-zone' ),
            'pro'  => __( 'yes','charity-zone' ),
        ),
        array(
            'desc'=> __( 'Hide Logo Text', 'charity-zone' ),
            'free' => __( 'yes','charity-zone' ),
            'pro'  => __( 'yes','charity-zone' ),
        ),
        array(
            'desc'=> __( 'Premium Support', 'charity-zone' ),
            'free' => __( 'no','charity-zone' ),
            'pro'  => __( 'yes','charity-zone' ),
        ),
        array(
            'desc'=> __( 'Fully SEO Optimized', 'charity-zone' ),
            'free' => __( 'no','charity-zone' ),
            'pro'  => __( 'yes','charity-zone' ),
        ),
        array(
            'desc'=> __( 'Recent Posts Widget', 'charity-zone' ),
            'free' => __( 'no','charity-zone' ),
            'pro'  => __( 'yes','charity-zone' ),
        ),
        array(
            'desc'=> __( 'Easy Google Fonts', 'charity-zone' ),
            'free' => __( 'no','charity-zone' ),
            'pro'  => __( 'yes','charity-zone' ),
        ),
        array(
            'desc'=> __( 'Pagespeed Plugin', 'charity-zone' ),
            'free' => __( 'no','charity-zone' ),
            'pro'  => __( 'yes','charity-zone' ),
        ),
        array(
            'desc'=> __( 'Only Show Header Image On Front Page', 'charity-zone' ),
            'free' => __( 'no','charity-zone' ),
            'pro'  => __( 'yes','charity-zone' ),
        ),
        array(
            'desc'=> __( 'Show Header Everywhere', 'charity-zone' ),
            'free' => __( 'no','charity-zone' ),
            'pro'  => __( 'yes','charity-zone' ),
        ),
        array(
            'desc'=> __( 'Custom Text On Header Image', 'charity-zone' ),
            'free' => __( 'no','charity-zone' ),
            'pro'  => __( 'yes','charity-zone' ),
        ),
        array(
            'desc'=> __( 'Full Width (Hide Sidebar)', 'charity-zone' ),
            'free' => __( 'no','charity-zone' ),
            'pro'  => __( 'yes','charity-zone' ),
        ),
        array(
            'desc'=> __( 'Only Show Upper Widgets On Front Page', 'charity-zone' ),
            'free' => __( 'no','charity-zone' ),
            'pro'  => __( 'yes','charity-zone' ),
        ),
        array(
            'desc'=> __( 'Replace Copyright Text', 'charity-zone' ),
            'free' => __( 'no','charity-zone' ),
            'pro'  => __( 'yes','charity-zone' ),
        ),
        array(
            'desc'=> __( 'Customize Upper Widgets Colors', 'charity-zone' ),
            'free' => __( 'no','charity-zone' ),
            'pro'  => __( 'yes','charity-zone' ),
        ),
        array(
            'desc'=> __( 'Customize Navigation Color', 'charity-zone' ),
            'free' => __( 'no','charity-zone' ),
            'pro'  => __( 'yes','charity-zone' ),
        ),
        array(
            'desc'=> __( 'Customize Post/Page Color', 'charity-zone' ),
            'free' => __( 'no','charity-zone' ),
            'pro'  => __( 'yes','charity-zone' ),
        ),
        array(
            'desc'=> __( 'Customize Blog Feed Color', 'charity-zone' ),
            'free' => __( 'no','charity-zone' ),
            'pro'  => __( 'yes','charity-zone' ),
        ),
        array(
            'desc'=> __( 'Customize Footer Color', 'charity-zone' ),
            'free' => __( 'no','charity-zone' ),
            'pro'  => __( 'yes','charity-zone' ),
        ),
        array(
            'desc'=> __( 'Customize Sidebar Color', 'charity-zone' ),
            'free' => __( 'no','charity-zone' ),
            'pro'  => __( 'yes','charity-zone' ),
        ),
        array(
            'desc'=> __( 'Customize Background Color', 'charity-zone' ),
            'free' => __( 'no','charity-zone' ),
            'pro'  => __( 'yes','charity-zone' ),
        ),
        array(
            'desc'=> __( 'Importable Demo Content', 'charity-zone' ),
            'free' => __( 'yes','charity-zone' ),
            'pro'  => __( 'yes','charity-zone' ),
        ),
    ),
    ...
    'charity_zone_recommended_plugins' => array(
...
return new charity_zone_Theme_Info( $config );