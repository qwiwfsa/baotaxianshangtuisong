...
class Charity_Zone_Button extends WP_Customize_Section {
	...
	public function json() {
		...
		if ( ! $this->button_url && $charity_zone_theme->get( 'ThemeURI' ) ) {

			$button_url = $charity_zone_theme->get( 'ThemeURI' );

		// Fall back to the `Author URI` defined in `style.css`.
		} elseif ( ! $this->button_url && $charity_zone_theme->get( 'AuthorURI' ) ) {

			$button_url = $charity_zone_theme->get( 'AuthorURI' );
		}

		$json['button_text'] = $this->button_text ? $this->button_text : $charity_zone_theme->get( 'Name' );
