...
class BackgroundComponent extends Component {
	constructor(props) {
		...
		this.onGradientChangeComplete =
			this.onGradientChangeComplete.bind(this);
	...
	onImageRemove(device) {
		if (
			undefined !== this.props.control.params.attachment &&
			this.props.control.params.attachment &&
			undefined !== this.props.control.params.attachment[device]
		) {
	...
	render() {
		...
		const dimensions = {
			desktop: {
				width:
					undefined !== this.controlParams.attachments &&
					"object" === typeof this.controlParams.attachments &&
					undefined !== this.controlParams.attachments.desktop &&
					"object" ===
						typeof this.controlParams.attachments.desktop &&
					this.controlParams.attachments.desktop &&
					undefined !== this.controlParams.attachments.desktop.width
						? this.controlParams.attachments.desktop.width
						: 400,
				height:
					undefined !== this.controlParams.attachments &&
					"object" === typeof this.controlParams.attachments &&
					...
					"object" ===
						typeof this.controlParams.attachments.desktop &&
					this.controlParams.attachments.desktop &&
					undefined !== this.controlParams.attachments.desktop.height
						? this.controlParams.attachments.desktop.height
						: 400,
			},
			tablet: {
				width:
					undefined !== this.controlParams.attachments &&
					"object" === typeof this.controlParams.attachments &&
					undefined !== this.controlParams.attachments.tablet &&
					"object" === typeof this.controlParams.attachments.tablet &&
					this.controlParams.attachments.tablet &&
					undefined !== this.controlParams.attachments.tablet.width
						? this.controlParams.attachments.tablet.width
						: 400,
				height:
					undefined !== this.controlParams.attachments &&
					"object" === typeof this.controlParams.attachments &&
					...
					"object" === typeof this.controlParams.attachments.tablet &&
					this.controlParams.attachments.tablet &&
					undefined !== this.controlParams.attachments.tablet.height
						? this.controlParams.attachments.tablet.height
						: 400,
			},
			mobile: {
				width:
					undefined !== this.controlParams.attachments &&
					"object" === typeof this.controlParams.attachments &&
					undefined !== this.controlParams.attachments.mobile &&
					"object" === typeof this.controlParams.attachments.mobile &&
					this.controlParams.attachments.mobile &&
					undefined !== this.controlParams.attachments.mobile.width
						? this.controlParams.attachments.mobile.width
						: 400,
				height:
					undefined !== this.controlParams.attachments &&
					"object" === typeof this.controlParams.attachments &&
					...
					"object" === typeof this.controlParams.attachments.mobile &&
					this.controlParams.attachments.mobile &&
					undefined !== this.controlParams.attachments.mobile.height
						? this.controlParams.attachments.mobile.height
						: 400,
			},
		};
		const getColorValue = (device) => {
			...
			if (undefined === this.state.value[device]) {
				...
				if (
					undefined !== this.state.value[largerDevice] &&
					undefined !== this.state.value[largerDevice].color &&
					this.state.value[largerDevice].color
				) {
					const paletteIndex =
						this.state.value[largerDevice].color.match(
							/\d+$/
						)?.[0] - 1;
					if (
						this.state.value[largerDevice].color.includes("palette")
					) {
						color =
							this.state.colorPalette[this.state.activePalette][
								paletteIndex
							].color;
					} else {
					...
					if (
						...
						this.state.value["desktop"].color.includes("palette")
					) {
						color =
							this.state.colorPalette[this.state.activePalette][
								paletteIndex
				...
				"" !== this.state.value[device].color &&
				null !== this.state.value[device].color &&
				this.state.value[device].color.includes("palette") &&
				this.state.colorPalette[this.state.activePalette] &&
				this.state.colorPalette[this.state.activePalette][
					devicePaletteIndex
				]
		...
		const getColorPreviewValue = (device) => {
			...
			if (undefined === this.state.value[device]) {
				...
				if (
					...
					color = this.state.value[largerDevice].color.includes(
						"palette"
					)
						? "var(--global-" +
						  this.state.value[largerDevice].color +
					...
					color = this.state.value["desktop"].color.includes(
						"palette"
					)
						? "var(--global-" +
						  this.state.value["desktop"].color +
		...
		const getGradientPreviewValue = (device) => {
			...
			if (undefined === this.state.value[device]) {
				...
				if (
					undefined !== this.state.value[largerDevice] &&
					undefined !== this.state.value[largerDevice].gradient &&
					this.state.value[largerDevice].gradient
				) {
		...
		const showImagePreview = (device) => {
			...
			if (undefined === this.state.value[device]) {
				...
				if (device === "mobile") {
					if (
						undefined !== this.state.value["tablet"] &&
						undefined !== this.state.value["tablet"].color &&
						"" !== this.state.value["tablet"].color
					) {
		...
		const showGradientPreview = (device) => {
			...
			if (undefined === this.state.value[device]) {
				...
				if (device === "mobile") {
					if (
						undefined !== this.state.value["tablet"] &&
						undefined !== this.state.value["tablet"].type &&
						"" !== this.state.value["tablet"].type
					) {
		...
		const getImagePreview = (device) => {
			...
			if (undefined === this.state.value[device]) {
				...
				if (
					undefined !== this.state.value[largerDevice] &&
					undefined !== this.state.value[largerDevice].image &&
					this.state.value[largerDevice].image.url
				) {
					imagePreview = (
						<img
							className="kadence-background-image-preview"
							src={
								undefined !== this.controlParams.attachments &&
								undefined !==
									this.controlParams.attachments[
										largerDevice
									] &&
								undefined !==
									this.controlParams.attachments[largerDevice]
										.thumbnail
									? this.controlParams.attachments[
											largerDevice
									  ].thumbnail
					...
					"tablet" === largerDevice &&
					undefined !== this.state.value["desktop"].image &&
					this.state.value["desktop"].image.url
				) {
					imagePreview = (
						<img
							className="kadence-background-image-preview"
				...
				imagePreview = (
					<img
						...
						src={
							undefined !== this.controlParams.attachments &&
							undefined !==
								this.controlParams.attachments[device] &&
							this.controlParams.attachments[device] &&
							undefined !==
								this.controlParams.attachments[device].thumbnail
								? this.controlParams.attachments[device]
										.thumbnail
								: this.state.value[device].image.url
						}
					/>
				);
			} else {
				...
				{this.state.currentDevice === "desktop" && (
					<Tooltip text={__("Reset Values", "kadence")}>
						<Button
							...
							disabled={
								this.state.value.desktop.color ===
									this.defaultValue.desktop.color &&
								undefined === this.state.value.desktop.type
							}
							onClick={() => {
								this.resetValues();
							}}
		...
		let initial_tab =
			...
			undefined !== this.state.value[this.state.currentDevice].type
						...
						<Tooltip
							text={
								this.controlParams.tooltip
									? this.controlParams.tooltip
									: __("Select Background", "kadence")
							}
						>
							<div className="background-button-wrap">
								<Button
									className={
										"kadence-background-icon-indicate"
									}
									onClick={() => {
										this.state.isVisible
											? toggleClose()
									...
									{(undefined ===
										...
										<Fragment>
											<ColorIndicator
												...
												colorValue={getColorPreviewValue(
													this.state.currentDevice
												)}
											/>
											{undefined !== colorString &&
												"" !== colorString &&
												null !== colorString &&
												"var" ===
											...
											{showGradientPreview(
												...
												<ColorIndicator
													className="kadence-advanced-color-indicate"
													colorValue={getGradientPreviewValue(
														this.state.currentDevice
													)}
												/>
											)}
