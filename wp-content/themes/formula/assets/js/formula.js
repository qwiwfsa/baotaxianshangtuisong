...
const navbarMenu = document.querySelector('.menu-modal.navbar');
...
function toggleMenu() {
    ...
    var isActive = navbarMenu.classList.contains('active');
    if (isActive) {
        // Close
        navbarMenu.classList.remove('active');
        setTimeout(function() { navbarMenu.style.display = 'none'; }, 250);
    } else {
        // Open
...
// Bind hamburger open button
document.addEventListener('DOMContentLoaded', function() {
    var openBtn = document.querySelector('.nav-toggle.mobile-nav-toggle');
    if (openBtn) {
    ...
    var closeBtn = document.querySelector('.menu-modal .close-nav-toggle');
    if (closeBtn) {
    ...
    jQuery('.sub-menu-toggle').on('click', function(e) {
        ...
        // Toggle the active class for rotation of the chevron
        jQuery(this).toggleClass('active');
        
        // Traverse up to the parent li, then find the child ul and toggle it smoothly
        jQuery(this).closest('li').children('ul').slideToggle(250);
    });
});
