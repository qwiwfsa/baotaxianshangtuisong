	...
	control: {
		...
		'formula-color': {
			...
			init: function ( control ) {
				...
				// Init the control.
				formula.input.color.init( control );
			},
			...
			template: function ( control ) {
				control.container.html(
					formula.input.color.getTemplate( {
		...
		color: {
			...
			getTemplate: function ( data ) {
				...
				html +=
					...
					'" value="' +
					data.value +
					'" class="formula-color-control" data-id="' +
					data['data-id'] +
					'"/>';

				return (
					'<div class="formula-input-container" data-id="' +
					data.id +
					'">' +
					html +
					'</div>'
			...
			init: function ( control ) {
				var picker = jQuery(
					'.formula-color-control[data-id="' + control.id + '"]'
				    ),
				    clear;
				...
				setTimeout( function () {
					clear = jQuery(
						'.formula-input-container[data-id="' +
						control.id +
						'"] .wp-picker-clear'
					);
					if ( clear.length ) {
...
(
	function () {
		 ...
		 */
		wp.customize.formulaDynamicControl = wp.customize.Control.extend( {
			initialize: function ( id, options ) {
				...
				if ( !args.params.type ) {
					args.params.type = 'formula-generic';
				}
			...
			ready: function () {
				...
				control.deferred.embedded.done( function () {
					control.initformulaControl( control );
				} );
			},
			...
			embed: function () {
				...
				wp.customize.section( sectionId, function ( section ) {
					if (
						'formula-expanded' === section.params.type ||
						section.expanded() ||
			 ...
			 * @returns {void}
			 */
			initformulaControl: function ( control ) {
				if ( 'undefined' !== typeof formula.control[control.params.type] ) {
					formula.control[control.params.type].init( control );
					return;
				}
			...
			formulaValidateCSSValue: function ( value ) {
				var validUnits = [
					    'rem',
					    'em',
					    'ex',
...
_.each( formula.control, function ( obj, type ) {
	wp.customize.controlConstructor[
		type
		] = wp.customize.formulaDynamicControl.extend( {} );
} );
 ...
 * Control: Slider.
 */
wp.customize.controlConstructor['formula-slider'] = wp.customize.Control.extend( {
	...
	ready: function () {
		...
		var control      = this,
		    ...
		    slider       = control.container.find( '.formula-slider' ),
		...
		// Init the control.
		if ( !_.isUndefined( window.formulaControlLoader ) && _.isFunction( formulaControlLoader ) ) {
			formulaControlLoader( control );
		} else {
			control.initformulaControl();
		}
	},

	initformulaControl: function () {
 ...
 * Control: Text.
 */
wp.customize.controlConstructor['formula-text'] = wp.customize.Control.extend( {
 ...
 * Control: Sortable.
 */
/* global formulaControlLoader */
wp.customize.controlConstructor['formula-sortable'] = wp.customize.Control.extend( {
	...
	ready: function () {
		...
		// Init the control.
		if (
			!_.isUndefined( window.formulaControlLoader ) &&
			_.isFunction( formulaControlLoader )
		) {
			formulaControlLoader( control );
		} else {
			control.initformulaControl();
	...
	initformulaControl: function () {
		...
		var control = this;
		control.container.find( '.formula-controls-loading-spinner' ).hide();
 ...
 * Control: Radio image.
 */
wp.customize.controlConstructor['formula-radio-image'] = wp.customize.formulaDynamicControl.extend( {} );

/**
 * Control: Toggle.
 */
wp.customize.controlConstructor['formula-toggle'] = wp.customize.formulaDynamicControl.extend( {
 ...
 */
wp.customize.controlConstructor['formula-radio-buttonset'] = wp.customize.Control.extend( {} );

/**
 * Control: Dimensions.
 */
;
/* global dimensionsformulaL10n */
/* global formulaControlLoader */
wp.customize.controlConstructor['formula-dimensions'] = wp.customize.formulaDynamicControl.extend( {
			...
			setting.bind( function ( value ) {
				...
				_.each( ['top', 'right', 'bottom', 'left'], function ( direction ) {
					if ( !_.isUndefined( value[direction] ) ) {
						if ( false === control.formulaValidateCSSValue( value[direction] ) ) {
							subs[direction] = dimensionsformulaL10n[direction];
						} else {
				...
				if ( !_.isEmpty( subs ) ) {
					message = dimensionsformulaL10n['invalid-value'] + ' (' + _.values( subs ).toString() + ') ';
...
;
/* global formulaControlLoader */
wp.customize.controlConstructor['formula-background'] = wp.customize.Control.extend( {
	...
	initformulaControl: function () {
		...
		var control = this,
		    value   = control.setting._value,
		    picker  = control.container.find( '.formula-color-control' );
 ...
 * Control: Editor.
 */
wp.customize.controlConstructor['formula-editor'] = wp.customize.formulaDynamicControl.extend( {
	...
	initformulaControl: function () {
		...
		var id = 'formula-editor-' + control_id;
