...
try {
    ...
    $stmt = $db->prepare("SELECT id, title, company, amount, period, category, description, image, content, status, sort_order, created_at, updated_at, seo_title, seo_keywords, seo_description FROM cases WHERE id = ? LIMIT 1");
    ...
    $tagStmt = $db->prepare("SELECT t.id, t.name FROM tags t JOIN case_tags ct ON t.id = ct.tag_id WHERE ct.case_id = ?");

    $tagStmt->bind_param('i', $caseId);
