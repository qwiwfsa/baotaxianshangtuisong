...
try {
    $db = getDB();
    $stmt = $db->prepare("SELECT COUNT(*) FROM article_comments WHERE ip_address = ? AND created_at > DATE_SUB(NOW(), INTERVAL 1 HOUR)");
    ...
    $stmt = $db->prepare("INSERT INTO article_comments (article_id, nickname, content, ip_address, status) VALUES (?, ?, ?, ?, ?)");
    $stmt->bind_param('isssi', $article_id, $nickname, $content_text, $ip, $status);
    $stmt->execute();
